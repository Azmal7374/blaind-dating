...
const lookingForOptions = ["Friendship", "Dating", "Networking", "Other"];
...
const RegisterScreen = () => {
  ...
  const handleSubmit = async () => {
    if (
      ...
      !dob.month ||
      !dob.year ||
      profilePics.length < 2
    ) {
      Alert.alert(
        "Error",
        "Please fill all the required fields and upload at least 2 profile pictures."
      );
      return;
    }
    ...
    try {
      const response = await axios.post(
        "http://192.168.1.102:8000/api/users/register",
        formData,
        {
          headers: {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
              ...
              <FlatList
                ...
                renderItem={({ item, index }) => (
                  <TouchableOpacity
                    style={[
                      styles.dateItem,
                      dob.month === (index + 1).toString() &&
                        styles.selectedDateItem,
                    ]}
                    onPress={() =>
                      setDob({ ...dob, month: (index + 1).toString() })
                    }
                  >
                    <Text
                      style={[
      ...
      <Animatable.View animation="fadeInDown" duration={1000} delay={1200}>
        ...
        >
          <Text>{lookingFor || "What are you looking for?"}</Text>
        </TouchableOpacity>
      </Animatable.View>

      {/* Looking For Modal */}
      <Modal visible={showLookingFor} transparent={true} animationType="slide">
        <Animatable.View
          animation="fadeIn"
          duration={300}
          style={styles.modalContainer}
          ...
          <View style={styles.modalContent}>
            ...
            {lookingForOptions.map((option) => (
              <TouchableOpacity
                key={option}
                style={[styles.modalButton, styles.genderButton]}
                onPress={() => handleLookingForSelect(option)}
              >
      ...
      {/* Interests Modal */}
      <Modal visible={showInterests} transparent={true} animationType="slide">
        <Animatable.View
          animation="fadeIn"
          duration={300}
          style={styles.modalContainer}
        >
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Select Interests (min 5)</Text>
